...
(function () {
  ...
  // 충돌(이동 불가)은 물(청초호·바다)만. 건물 박스는 진입 지점을 덮어 "학교 안 들어가짐" 등을
  // 유발하므로 제거 — 건물은 걸어서 다가가 진입(존/탭). 못 움직이는 영역 최소화.
  // 물 충돌도 작게 — 깊은 물만 막고 호숫가/해변 입구는 드러나 걸어 닿게.
  const WORLD_SOLIDS = [
    SB(0.80, 0.83, 0.08, 0.06),  // 청초호 깊은 물(작게) — 입구(0.75,0.78)는 노출
    SB(0.975, 0.5, 0.045, 1.0),  // 동해 바다(아주 우측 띠) — 해변 입구(0.93) 노출
  ];
  ...
  function worldDef() {
    ...
    const portals = PLACES.filter((p) => WORLD_LOC[p.id]).map((p) => {
      // 진입 존은 건물 바로 앞에서만 걸리게 좁게. (탭 진입 + 밖→안 에지 트리거라 작아도 됨)
      const big = (p.id === "lake" || p.id === "beach") ? 150 : 110;
